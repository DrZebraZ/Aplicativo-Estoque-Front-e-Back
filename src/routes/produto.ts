...
async function produtos(server:any){
  ...
  server.delete('/deletaProduto',{schema:{body:$ref('deletaProdutoSchema')}},deletaProduto);
  }
...
async function deletaProduto(request:FastifyRequest<{Body: deletaProdutoSchema}>, reply:FastifyReply){
  try{
    const produto = request.body
